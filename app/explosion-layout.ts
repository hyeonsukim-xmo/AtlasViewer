export interface BoundedPart {
  id: string;
  bounds: [number[], number[]];
}
export const EXPLOSION_DURATION_MS = 1700;
export function transitionProgress(elapsed: number, duration = EXPLOSION_DURATION_MS) {
...
export function createExplosionLayout(parts: BoundedPart[], aspect = 1) {
  const cards = parts.map((p) => ({
    ...
    height: Math.max(0.035, p.bounds[1][1] - p.bounds[0][1]) + 0.08,
  }));
